...
languages, totals stars and recently-active repos (measured against `SOURCE_DATE_EPOCH`,
never wall-clock, so the count stays reproducible), groups by the first topic, and writes
Markdown. See [`EXAMPLE.md`](./EXAMPLE.md) for live output. Sole dependency: `zod`.
